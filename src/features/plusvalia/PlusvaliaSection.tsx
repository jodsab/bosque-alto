...
export default function PlusvaliaSection() {
  ...
  return (
    <section id="plusvalia" className="bg-brand-dark py-section-y">
      <div className="mx-auto max-w-6xl px-5">
        <p className="eyebrow-on-dark">{plusvalia.eyebrow}</p>
        <h2 className="mt-2 max-w-3xl font-display text-3xl text-text-on-dark sm:text-4xl">{plusvalia.title}</h2>
        <p className="mt-3 max-w-2xl text-sm text-text-on-dark-muted sm:text-base">{plusvalia.description}</p>

        <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {(plusvalia.facts as PlusvaliaFact[]).map((fact) => (
            <div key={fact.title} className="rounded-lg border border-white/10 bg-white/5 p-6">
              <p className="font-display text-2xl text-accent-gold">
                {fact.value}
                {fact.unit && <span className="ml-1 text-sm text-text-on-dark-muted">{fact.unit}</span>}
              </p>
              <h3 className="mt-2 font-semibold text-text-on-dark">{fact.title}</h3>
              <p className="mt-2 text-sm text-text-on-dark-muted">{fact.detail}</p>
              {fact.note && <p className="mt-2 text-xs text-accent-gold">{fact.note}</p>}
              {fact.sourceUrl && (
                <a
                  href={fact.sourceUrl}
                  target="_blank"
        ...
        <div className="mt-10 border-t border-white/10 pt-10">
          ...
          <p className="mt-2 font-display text-lg text-accent-gold sm:text-xl">{plusvalia.caveat.highlight}</p>
